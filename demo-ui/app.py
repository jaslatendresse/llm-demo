...
from prompts.utils import PromptsContainer
        ...
        for i in range(int(len(tab_data) / 2) + 1):
            ...
            with row:
                for j in range(2):
                    ...
                    with gr.Group():
                        gr.HTML(
                            f'<p style="color: white; font-weight: bold; padding-left: 1px;">{item["act"]}</p>'
                        )
        ...
        with gr.Row(equal_height=True):
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    ...
                    undo_button = gr.Button("↩️ Cancel", variant="secondary")
                    clear_button = gr.Button("🗑️  Erase", variant="secondary")
